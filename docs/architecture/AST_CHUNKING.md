...
flowchart TD
    ...
    S -->|no| G{Grammar known?<br/>py · js · ts · tsx}
...
2. **Known grammars split by definition.** For Python, JavaScript, TypeScript,
   and TSX, each top-level function and class becomes its own chunk, at its real
   line range. A definition longer than 200 lines is windowed so no chunk grows
   unbounded.
...
## Why tree-sitter (and only four grammars)
...
`tree-sitter-language-pack`. We wire up the four languages this codebase and its
fixtures actually use first; adding Java or Kotlin later is a one-line entry in
the grammar table plus its definition node types. Languages we do not wire up
keep working through the line-window fallback.

## What this is not (yet)

No method-level splitting inside a large class (the class is one chunk until it
crosses the 200-line cap). No import/dependency graph — that is a separate
backlog item. No incremental re-indexing — a re-index still rebuilds every
chunk.
