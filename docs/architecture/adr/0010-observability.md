...
standard; production telemetry (metrics, dashboards, alerting) is M7 scope.
...
- **LLM tracing:** every ModelRouter call records model, latency, token usage, and cost.
  **Langfuse (self-hosted)** becomes the trace sink when agent runs land (M1) — spans
  per run → step → model call.
- **OpenTelemetry-shaped from the start:** ids and span-like structure now; the actual
  OTel SDK/collector + Prometheus/Grafana stack arrives in M7 rather than carrying that
...
- Langfuse joins compose in M1; its absence must never break the engine (fire-and-forget
  exporter).
- M7 revisits this ADR to wire the OTel SDK and metrics/alerting.
