...
- **Extract** — for each Python, JavaScript, TypeScript, and TSX file,
  tree-sitter finds the real `import` / `from … import` statements (not ones
  hiding in comments or strings). `engine/indexing/dependency_graph.py`.
- **Resolve** — an import is kept only when it points at *another file in the
  ...
  extensions and `index` files. Imports of third-party packages resolve to
  nothing and are dropped — the graph is the repository's *own* structure.
...
folder are later refinements. Only relative/first-party imports are drawn — the
graph is about the repository's own architecture, not its package manifest.
