...
| 7 | Row-level security | Postgres refuses a pinned session another user's rows; the engine role is NOSUPERUSER | audited in its own slice — [ROW_LEVEL_SECURITY.md](../architecture/security/ROW_LEVEL_SECURITY.md); suite runs under FORCE RLS; superuser fails the suite loudly | **Holds** (2026-07-17 deny-by-default; 2026-07-19 privilege separation — the service clause requires the `asep` role, and user-pinned sessions run as the non-owner `asep_api`) |
...
- ~~Unset RLS context is the trusted internal path~~ — **closed 2026-07-17**:
  ...
  ([ROW_LEVEL_SECURITY.md](../architecture/security/ROW_LEVEL_SECURITY.md)). The
  remaining, narrower boundary — an attacker setting the flag themselves —
  **closed 2026-07-19**: the service clause requires *being* the `asep`
  role, and user-pinned sessions connect as the non-owner `asep_api`, which
  cannot touch policies and gains nothing from the GUC.
