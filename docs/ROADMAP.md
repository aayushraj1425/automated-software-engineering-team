...
| **6** | **Workspace & Integrations** *(documentation suite with git-history changelog and in-place editing; Slack / Linear / Jira / GitLab / Bitbucket integrations; and the run-workspace file browser + editor + git commit/push panel shipped; the in-browser terminal remains)* | large | Editor / terminal / git panels; Jira, Linear, Slack; GitLab, Bitbucket; documentation generation suite (API docs, READMEs, changelogs, guides) | intelligent coding; workflow integrations |
